...
public static class Program
{
    ...
    {
        var appToken = File.ReadAllText("../../../ignore/BotUserOAuthToken.txt").Trim();
        var userToken = Secrets.UserToken;

        var channels = await ListUserChannels(userToken);
        ...
        // }
        //
        // var outputFilePath = "SlackMessages.txt";
    ...
    private static async Task<string> FetchSlackMessages(string token, string startDate, string endDate)
    {
        ...
        var url = $"https://slack.com/api/conversations.history?oldest={startDate}&latest={endDate}";
    ...
    private static async Task<string> ListUserChannels(string token)
    {
        ...
        var url = "https://slack.com/api/users.conversations";
        var response = await client.GetAsync(url);
    ...
    private static async Task<string> AuthenticateUser()
    {
        ...
        var authUrl = $"https://slack.com/oauth/v2/authorize?client_id={Secrets.AppClientId}&scope=channels:read,groups:read,im:read,mpim:read&redirect_uri={redirectUri}";
        Console.WriteLine("Please open the following URL in your browser to authenticate:");
        Console.WriteLine(authUrl);

        using var listener = new HttpListener();
        listener.Prefixes.Add("https://localhost:5000/callback/");
        listener.Start();
        ...
        await writer.WriteLineAsync("Authentication successful! You can close this window.");
        context.Response.Close();

        var tokenResponse = await Client.PostAsync("https://slack.com/api/oauth.v2.access", new FormUrlEncodedContent(new[]
        {
            new KeyValuePair<string, string>("client_id", Secrets.AppClientId),
            new KeyValuePair<string, string>("client_secret", Secrets.AppClientSecret),
